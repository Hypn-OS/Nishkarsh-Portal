                ...
                <div class="col-md-6 offset-md-3 card shadow my-md-4 py-3">
                    ...
                    <form @submit.prevent="registerOrganization" v-if="!loading">
                        <div class="form-group">
                            <label for="UdyogID">Udyog ID*</label>
                            <input v-model="udyogId" type="text" class="form-control" id="UdyogID" placeholder="Enter Udyog ID">
                        </div>
                        <div class="form-group">
...
export default {
    ...
    methods: {
        verifyUdyogId(){
            return firebase.firestore().collection("udyogID").where("id", "==", this.udyogId).get();
        },
        registerOrganization(){
                ...
                .then(snapshot => {
                    if(snapshot.empty){
                        this.loading = false;
                        throw 'Invalid Udyog ID';
                    }else{
                        snapshot.docs.forEach(doc => {
                            orgObj.state = doc.data().state;
                            orgObj.expertise = doc.data().expertise;
                        });
